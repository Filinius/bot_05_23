...
class PandasCalc:
    ...
    def calc_result_reps(self, exercise, exercise_result):
        if exercise_result > self.df[exercise].min():  # проверяет, больше ли значение переменной "exercise_result" минимального значения в столбце "exercise" в df
            if not any(self.df[exercise].eq(exercise_result)):  # есть ли в столбце "exercise" какое-либо значение, равное переменной "exercise_result"
                next_exercise_result = self.df.loc[self.df[exercise] < exercise_result, exercise].iloc[0]  # (знак < для значечий, где КОЛИЧЕСТВО повторений в упражнении) переменная "next_exercise_result" будет содержать значение первой строки в столбце "exercise", которое меньше значения переменной "exercise_result"
                exercise_points = self.df.loc[self.df[exercise] == next_exercise_result, 'points'].iloc[0]  # получаем баллы
                return exercise_points
            else:
                exercise_points = self.df.loc[self.df[exercise] == exercise_result, 'points'].iloc[0]  # если есть значение полученного результата, то находим количество баллов
                return exercise_points
        else:
