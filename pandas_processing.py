...
if exercise_result > df[exercise].min(): # проверяем что б значение результата было больше минимального значения в таблице
    if not any(df[exercise] == exercise_result): # проверяем если нет введеного значения в столбце с результатами упражнений
        next_exercise_result = df.loc[df[exercise] < exercise_result, exercise].iloc[0] #(знак < для значечий КОЛИЧЕСТВО) если нет полученного результата в колонке, то берем тот результат который меньше нашего
